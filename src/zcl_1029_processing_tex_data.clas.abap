CLASS zcl_1029_processing_tex_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
**********************************************************************
     ...
     "Date
     DATA(text2) = |Raw Date: { the_date             }|.
     DATA(text3) = |ISO Date: { the_date Date = ISO  }|.
     DATA(text4) = |USER Date:{ the_date Date = USER }|.
...
* Output
**********************************************************************
    out->write( 'DATE FORMATS IN ABAP' ).
    out->write( | ORIGINAL TEXT: { text } | ).
    out->write( | Original Date TEXT2: { text2 } |  ).
